...
export default function DocumentsPage() {
  ...
  const category: CategoryFilter = rawCat ?? 'ALL';
  ...
  // 폴링: PROCESSING 문서가 있을 때 3초마다 갱신 → 쿼리 캐시 갱신 후 useDocuments에 반영
  ...
  const handleCategoryChange = (cat: CategoryFilter) => {
    ...
    router.replace(`/couple/documents?${params.toString()}`);
  };
  ...
  return (
    <div className={styles.page}>
      <LoadingModal open={uploadMutation.isPending} message="문서를 업로드하고 있어요..." />

      <main className={styles.main}>
        <section className={styles.card}>
          <header className={styles.header}>
            <h1 className={styles.pageTitle}>문서 보관함</h1>
            <p className={styles.subTitle}>계약서·영수증 등 웨딩 문서를 AI가 자동 분류해요</p>
          </header>
