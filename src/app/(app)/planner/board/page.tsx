...
// couple/community와 동일한 레이아웃·스타일 재사용
import styles from '@/app/(app)/couple/community/page.module.css';

const MAX_RECENT  = 10;
const STORAGE_KEY = 'planner_board_recent_searches';
...
export default function PlannerBoardPage() {
  ...
  const closeSearch = () => { setShowSearch(false); setSearchKw(''); };
        ...
        <main className={styles.main}>
          <div className={styles.headerBar}>
            <div className={styles.titleArea}>
              <h1 className={styles.pageTitle}>플래너 게시판</h1>
              <p className={styles.pageSub}>동료 플래너들과 정보를 공유해요</p>
            </div>
            <div className={styles.headerActions}>
              <button type="button" className={styles.searchBtn} onClick={openSearch} aria-label="검색">
                <Search size={15} />
              </button>
              <button type="button" className={styles.writeBtn} onClick={() => router.push('/planner/board/new')}>
                <PenSquare size={15} />
                글쓰기
              </button>
            </div>
          ...
          {!isLoading && !isError && posts.length === 0 && (
            <div className={styles.empty}>
              ...
              <p className={styles.emptyDesc}>
                {tags.length > 0 ? '선택한 태그의 게시글이 없습니다.' : '첫 번째 게시글을 작성해보세요!'}
              </p>
              <button type="button" className={styles.emptyBtn} onClick={() => router.push('/planner/board/new')}>
                글쓰기
              </button>
            </div>
          )}
          ...
          {posts.length > 0 && (
            <div className={styles.list}>
              {posts.map((post) => (
                <PostCard key={post.postId} post={post} basePath="/planner/board" />
              ))}
              <div ref={sentinelRef} />
              {isFetchingNextPage && <div className={styles.skeletonCard} />}
            </div>
          )}
        </main>
      </div>
      ...
      {showSearch && (
        <div className={styles.searchOverlay}>
          <div className={styles.searchBar}>
            ...
            />
            <button type="button" className={styles.searchClose} onClick={closeSearch}>취소</button>
          </div>
            ...
            <div className={styles.recentSearches}>
              ...
              <div className={styles.recentList}>
                ...
                {recentSearches.map((kw) => (
                  <button key={kw} type="button" className={styles.recentChip}
