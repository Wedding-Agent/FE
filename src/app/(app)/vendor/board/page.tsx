...
const MAX_RECENT  = 10;
const STORAGE_KEY = 'vendor_board_recent_searches';
        ...
        <main className={styles.main}>
          <div className={styles.headerBar}>
            <div className={styles.titleArea}>
              <h1 className={styles.pageTitle}>업계 게시판</h1>
              <p className={styles.pageSub}>동종 업계 정보를 공유해요</p>
            </div>
            <div className={styles.headerActions}>
              <button type="button" className={styles.searchBtn} onClick={openSearch} aria-label="검색">
                <Search size={15} />
              </button>
              <button type="button" className={styles.writeBtn} onClick={() => router.push('/vendor/board/new')}>
                <PenSquare size={15} />
                글쓰기
              </button>
            </div>
          ...
          {!isLoading && !isError && posts.length === 0 && (
            <div className={styles.empty}>
              ...
              <button type="button" className={styles.emptyBtn} onClick={() => router.push('/vendor/board/new')}>
                글쓰기
              </button>
            </div>
          )}
          ...
          {posts.length > 0 && (
            <div className={styles.list}>
              {posts.map((post) => (
                <PostCard key={post.postId} post={post} basePath="/vendor/board" />
              ))}
              <div ref={sentinelRef} />
